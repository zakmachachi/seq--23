...
![Prototype Image](./images/prototype_v2.jpeg)
>[!WARNING]
...
**Every kick is identical.** For a given set of knob settings every kick is bit-identical from its first sample, supersaw included, however it lands on the one before. The Daisy's kick output runs 10 ms behind: when a note arrives, the not-yet-played end of the old kick fades to silence and every oscillator and filter restarts from zero. The Digitakt passthrough (Out 2) is delayed by the same 10 ms so the two stay aligned. Everything the kick makes is high-passed at 25 Hz (12 dB/oct) just before the output.
